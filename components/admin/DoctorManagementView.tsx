...
type SortKey = 'name' | 'specialization' | 'mobileNo';

interface DoctorManagementViewProps {
    onViewUser: (user: User) => void;
}

const DoctorManagementView: React.FC<DoctorManagementViewProps> = ({ onViewUser }) => {
    ...
    const handleDeleteUser = async (user: User) => {
        if (window.confirm(`Are you sure you want to delete doctor "${user.name}" (${user.healthId})?`)) {
            try {
                await deleteUser(user.healthId);
                setAllUsers(prevUsers => prevUsers.filter(u => u.healthId !== user.healthId));
                setToastMessage(`Doctor ${user.name} has been deleted.`);
            } catch (err) {
    ...
    const sortedAndFilteredDoctors = useMemo(() => {
        let filtered = allUsers.filter(user => user.role === UserRole.DOCTOR);
        ...
        <div className="space-y-6">
            <div className="flex justify-between items-center">
                <div>
                    ...
                    <p className="text-md text-gray-500 dark:text-dark-subtext mt-1">Manage all doctor accounts on the platform.</p>
                </div>
                <button
                        ...
                        <table className="w-full text-left table-auto">
                            <thead>
                                <tr className="border-b border-gray-200 dark:border-dark-subtext/20 text-xs text-gray-500 dark:text-dark-subtext uppercase">
                                    ...
                                    <SortableHeader sortKey="specialization">Specialization</SortableHeader>
                                    <th className="p-3 hidden md:table-cell">Contact</th>
                                    <th className="p-3 text-right">Actions</th>
                                </tr>
                            </thead>
                            <tbody>
                                {sortedAndFilteredDoctors.map(user => (
                                    <tr 
                                        key={user.healthId} 
                                    ...
                                    >
                                        ...
                                        <td className="p-3 text-gray-500 dark:text-dark-subtext">{user.specialization || 'N/A'}</td>
                                        <td className="p-3 hidden md:table-cell text-gray-500 dark:text-dark-subtext">
                                            <div>{user.mobileNo || 'N/A'}</div>
             ...
             {isAddModalOpen && (
                <AddDoctorModal
                    ...
                    onSuccess={() => {
                        ...
                        setToastMessage('New doctor added successfully!');
                    }}
                />
             )}
        </div>
